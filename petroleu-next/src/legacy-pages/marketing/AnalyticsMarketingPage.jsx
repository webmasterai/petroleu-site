...
export default function AnalyticsMarketingPage() {
  const { isAfghanistan, mp, copy } = useUiCopy()
  const hero = usePageHero('analytics', {
    badge: 'Best Analytics & Insights',
    title: 'Real-Time Insights for Smarter Decisions',
    subtitle:
      'Powerful dashboards, instant variance detection, and shift-aware reports that turn raw pump data into clear daily action.',
  })
            ...
            <div className="mt-8 flex flex-col items-center justify-center gap-4 sm:flex-row">
              <Link to={mp('/contact')}>
                <MButton size="lg" className="gap-2">
                  {copy.start_trial || (isAfghanistan ? null : 'Start Free Trial') || 'Contact'}
                  <ArrowRight className="h-4 w-4 rtl:rotate-180" />
                </MButton>
              </Link>
              <Link to={mp('/contact')}>
                <MButton size="lg" variant="outline">
                  {copy.contact_sales || (isAfghanistan ? null : 'Request a Demo') || 'Contact'}
                </MButton>
              </Link>
            </div>
          </div>
