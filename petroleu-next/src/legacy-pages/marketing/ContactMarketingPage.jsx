...
export default function ContactMarketingPage() {
  ...
  const { data: heroData } = useCmsQuery(['hero', 'contact'], '/hero/contact')
  const { data: settingsRaw } = useCmsQuery(['settings'], '/settings')
  const { data: faqRaw } = useCmsQuery(['faq', 'contact'], '/faqs', {
    config: { params: { page: 'contact' } },
  })

  const settingsMap = settingsRaw && typeof settingsRaw === 'object' ? settingsRaw : {}
  const primaryEmail = market === 'af' ? (settingsMap.email || settingsMap.contact_email || null) : websiteContent.footer.email
  const phone = market === 'af' ? (settingsMap.phone || settingsMap.contact_phone || null) : websiteContent.footer.phone
  const phoneTel = market === 'af' ? (settingsMap.phone_tel || settingsMap.phoneTel || phone) : websiteContent.brand.phoneTel
  const address =
    settingsMap.address ||
    settingsMap.contact_address ||
    (market === 'af' ? null : websiteContent.footer.address)
