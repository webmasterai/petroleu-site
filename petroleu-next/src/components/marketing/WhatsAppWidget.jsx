...
export function WhatsAppWidget() {
  ...
  const { isAfghanistan, whatsappUrl: afUrl, settings } = useUiCopy()
  const { whatsappNumber, whatsappMessage, name } = websiteContent.brand
  const pkUrl = `https://wa.me/${whatsappNumber}?text=${encodeURIComponent(whatsappMessage)}`
  const whatsappUrl = isAfghanistan ? afUrl : pkUrl

  if (isAfghanistan && !whatsappUrl) return null

  const supportName = isAfghanistan ? settings.site_name || 'Petroleu' : name
  ...
  return (
    <div className="fixed bottom-6 end-6 z-50 flex flex-col items-end gap-3">
        ...
        <div className="mb-1 w-80 overflow-hidden rounded-2xl shadow-2xl border border-border bg-card">
          <div className="flex items-center gap-3 bg-[#25D366] px-4 py-4">
            <div className="relative">
              ...
              <span className="absolute bottom-0 end-0 h-3 w-3 rounded-full border-2 border-[#25D366] bg-green-300" />
            </div>
            <div className="flex-1">
              <p className="font-semibold text-white text-sm" dir="ltr">
                {supportName}
              </p>
            </div>
            <button
          ...
          <div className="bg-[#ECE5DD] px-4 py-5">
            <div className="max-w-[85%] rounded-2xl rounded-ss-none bg-white px-4 py-3 shadow-sm">
              <p className="text-sm text-gray-800 leading-relaxed">
                <strong dir="ltr">{supportName}</strong>
              </p>
            </div>
          </div>
      ...
      >
        {!isOpen && <span className="absolute inset-0 rounded-full bg-[#25D366] animate-ping opacity-40" />}
        {isOpen ? <X className="h-6 w-6 text-white" /> : <WhatsAppGlyph className="h-7 w-7 fill-white" />}
      </button>
    </div>
  )
}
