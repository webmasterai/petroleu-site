...
export default function AboutMarketingPage() {
  const q = useQueries({
    queries: [
      { queryKey: ['cms', 'hero', 'about'], queryFn: () => safeCmsGet('/hero/about') },
      { queryKey: ['cms', 'stats'], queryFn: () => safeCmsGet('/stats') },
      { queryKey: ['cms', 'mission'], queryFn: () => safeCmsGet('/mission-values') },
      { queryKey: ['cms', 'company-story'], queryFn: () => safeCmsGet('/company-story') },
      { queryKey: ['cms', 'team'], queryFn: () => safeCmsGet('/team') },
      { queryKey: ['cms', 'cta', 'about'], queryFn: () => safeCmsGet('/cta/about') },
    ],
  })
  const [heroQ, statsQ, missionQ, storyQ, teamQ, ctaQ] = q
  const heroData = heroQ.data
  const statsData = Array.isArray(statsQ.data) ? statsQ.data : []
  const missionData = Array.isArray(missionQ.data) ? missionQ.data : []
  const storyPayload = storyQ.data || {}
  const storyData = Array.isArray(storyPayload.story) ? storyPayload.story : []
  const achievementsData = Array.isArray(storyPayload.achievements) ? storyPayload.achievements : []
  const teamData = Array.isArray(teamQ.data) ? teamQ.data : []
