...
export default function AdminPricingPlansPage() {
    ...
    try {
      ...
      const payload = {
        ...
        data: {
          ...
          price_suffix: form.price_suffix || '/month',
          period: 'month',
          is_popular: form.is_popular,
          popular: form.is_popular,
          badge: form.badge || (form.is_popular ? 'Most Popular' : ''),
          currency: 'PKR',
          features,
  ...
  async function toggleEnabled(row) {
    try {
      await adminPut(`/sections/${row.id}`, {
        ...row,
        is_enabled: row.is_enabled === false,
        status: 'published',
      })
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <p className="max-w-xl text-sm text-muted-foreground">
            Pricing Plans for this market/locale. Edit title, price, features, badge, CTA. Disable or
            delete a plan — the public site removes it after publish (no hardcoded restore).
          </p>
        </div>
      ...
      {open ? (
        ...
        >
          <div className="grid gap-4 sm:grid-cols-2">
            ...
            <label className="block text-sm">
              <span className="mb-1.5 block text-sm font-medium">Price suffix</span>
              <input
                className="admin-input"
            ...
            <label className="block text-sm">
              <span className="mb-1.5 block text-sm font-medium">Badge (optional)</span>
              <input
                className="admin-input"
                value={form.badge}
                  ...
                  <tr key={row.id} className="border-t border-black/[0.05]">
                    ...
                    <td>
                      <span className="admin-chip">
                        {row.is_enabled === false ? 'Disabled' : row.status === 'published' ? 'Published' : 'Draft'}
                    ...
                    <td>
                      <div className="flex flex-wrap gap-2">
                        ...
                        >
                          {row.is_enabled === false ? 'Enable' : 'Disable'}
                        </button>
                        <button
                          type="button"
